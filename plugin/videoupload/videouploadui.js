...
const videoUploadKeystroke = 'Ctrl+Shift+K';
...
export default class VideoUploadUI extends Plugin {
	...
	init() {
		...
		this._createToolbarVideoUploadButton();
	...
	/**
	 * Creates a toolbar video upload button. Clicking this button will show
	 */
	_createToolbarVideoUploadButton() {
		const editor = this.editor;
		const videoUploadCommand = editor.commands.get( 'videoUpload' );
		const t = editor.t;

		// Handle the `Ctrl+Shift+K` keystroke and show the panel.
		editor.keystrokes.set( videoUploadKeystroke, ( keyEvtData, cancel ) => {
			// Prevent focusing the search bar in FF and opening new tab in Edge. @ckeditor5-link #153, #154.
			cancel();

			if ( videoUploadCommand.isEnabled ) {
				this._showUI();
			}
		} );
		...
		editor.ui.componentFactory.add( 'videoUpload', locale => {
			...
			button.label = t( 'Video upload' );
			button.icon = mediaIcon;
			button.keystroke = videoUploadKeystroke;
			button.tooltip = true;

			// Bind button to the command.
			button.bind( 'isEnabled' ).to( videoUploadCommand, 'isEnabled' );
	...
	/**
	 * Returns positioning options for the {@videoupload #_balloon}. They control the way the balloon is attached
	 * to the target element or selection.
	 *
	 * If the selection is collapsed and inside a video upload element, the panel will be attached to the
	 * entire video upload element. Otherwise, it will be attached to the selection.
	 */
